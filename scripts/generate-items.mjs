...
async function main() {
  ...
  for (const slug of itemSlugs) {
    ...
    try {
      ...
      consumable =
        itemData.attributes?.some((a) => a.name === "consumable") ?? false;
    ...
    if (version) {
      results.push({ slug, de: deName, en: enName, version, consumable });
      if (wasFuzzy) {
  ...
  const tsContent =
    ...
    `export const ITEMS: { slug: string; de: string; en: string; version: string; consumable: boolean }[] = ${JSON.stringify(results, null, 2)};\n`;
