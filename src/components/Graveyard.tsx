...
const Graveyard: React.FC<GraveyardProps> = ({ graveyard = [], playerNames, playerColors, onManualAddClick, generationSpritePath }) => {
  ...
  const names = useMemo(() => {
    ...
    return list.map((name, index) => (name?.trim().length ? name : t('graveyard.defaultPlayer', { index: index + 1 })));
  }, [playerNames]);
    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md border border-gray-300 dark:border-gray-700 overflow-hidden custom-scrollbar">
      ...
      <div className="p-4 max-h-96 overflow-y-auto">
        {graveyard && graveyard.length > 0 ? (
          <div className="space-y-3">
            {graveyard.map((pair) => (
              <div key={pair.id} className="p-2 border border-gray-200 dark:border-gray-600 rounded-md bg-gray-50 dark:bg-gray-700 text-xs">
                <p className="text-center font-bold text-gray-600 dark:text-gray-300 mb-1">{t('graveyard.areaLabel', { route: pair.route || t('common.unknownRoute') })}</p>
                  ...
                  {names.map((name, index) => {
                    ...
                    return (
                      <div key={`${pair.id}-player-${index}`}>
                        ...
                        <p className="text-gray-700 dark:text-gray-400">{t('graveyard.nicknameLabel', { nickname: member.nickname || t('graveyard.noNickname') })}</p>
                      </div>
                    );
                  })}
                </div>
          ...
          <p className="text-center text-gray-500 dark:text-gray-400 text-sm py-4">{t('graveyard.empty')}</p>
        )}
      </div>
    </div>
  );
