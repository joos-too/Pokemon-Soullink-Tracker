...
const TeamTable: React.FC<TeamTableProps> = ({ title, title2, color1, color2, data, onPokemonChange, onRouteChange, onAddToGraveyard, isTeam = false }) => {
  // FIX: Added colSpan to HeaderCell props to allow it to be passed to the th element.
  ...
  return (
    <div className="overflow-x-auto bg-white rounded-lg shadow-md border border-gray-300">
      <table className="w-full border-collapse">
        <thead>
          <tr>
            <HeaderCell color={color1} colSpan={isTeam ? 5 : 4}>{title}</HeaderCell>
            <HeaderCell color={color2} colSpan={4}>{title2}</HeaderCell>
            <th className="p-2 bg-green-600 text-white font-press-start text-[10px] text-center" style={{writingMode: isTeam ? 'vertical-rl' : 'horizontal-tb', textOrientation: isTeam ? 'mixed' : 'mixed'}}>Route</th>
            <th className="p-2 bg-gray-700"></th>
          </tr>
          <tr>
            <th className="p-2 text-xs font-bold text-center border-t border-gray-300">#</th>
            <th className="p-2 text-xs font-bold text-center border-t border-gray-300">Pokemon</th>
            <th className="p-2 text-xs font-bold text-center border-t border-gray-300">Nickname</th>
            <th className="p-2 text-xs font-bold text-center border-t border-gray-300" colSpan={2}>Typ</th>
            <th className="p-2 text-xs font-bold text-center border-t border-gray-300">Pokemon</th>
            <th className="p-2 text-xs font-bold text-center border-t border-gray-300">Nickname</th>
            <th className="p-2 text-xs font-bold text-center border-t border-gray-300" colSpan={2}>Typ</th>
        ...
        <tbody>
          {data.map((pair, index) => (
            <tr key={pair.id} className="border-t border-gray-200 hover:bg-gray-50">
              <td className="p-2 text-center font-bold text-sm border-r border-gray-200">{pair.id}</td>
              ...
              <td className="p-1 text-center">
                 {(pair.player1.name || pair.player2.name) && (
                    <button 
                      onClick={() => window.confirm('Move this pair to the graveyard?') && onAddToGraveyard(pair)}
