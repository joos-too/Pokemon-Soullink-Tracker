  ...
  return (
    <div>
      ...
      <div className="relative">
        ...
        />
        {spriteUrl ? (
          <img src={spriteUrl} alt="" aria-hidden="true" className="pointer-events-none absolute inset-y-0 right-2 my-auto h-8 w-8 select-none" loading="lazy" />
        ) : null}
        {open && suggestions.length > 0 && (
          <div className="absolute z-10 mt-1 w-full max-h-56 overflow-auto rounded-md border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-lg">
            {loading && (
              <div className="px-3 py-2 text-sm text-gray-500 dark:text-gray-400">Lade Vorschlaege...</div>
            )}
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-md">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold dark:text-gray-100">Verlorene Pokémon hinzufügen</h2>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mt-6 flex justify-end gap-2">
            <button type="button" onClick={onClose} className={`px-4 py-2 rounded-md border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700 ${focusRingClasses}`}>Abbrechen</button>
            <button type="submit" disabled={!isValid} className={`px-4 py-2 rounded-md font-semibold shadow ${isValid ? 'bg-green-600 text-white hover:bg-green-700' : 'bg-gray-300 dark:bg-gray-600 text-gray-500 dark:text-gray-400 cursor-not-allowed'} ${focusRingClasses}`}>
              Hinzufügen
            </button>
          </div>
        </form>
