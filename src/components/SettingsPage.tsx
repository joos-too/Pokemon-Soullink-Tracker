    ...
    <div className="bg-[#f0f0f0] dark:bg-gray-900 min-h-screen flex items-center justify-center p-4">
      <div className="w-full max-w-2xl mx-auto bg-white dark:bg-gray-800 shadow-lg p-6 rounded-lg">
        ...
        <main className="mt-6 space-y-8">
          ...
          <section>
            <h2 className="text-sm font-semibold uppercase tracking-[0.3em] text-gray-500 mb-4">Tracker Optionen</h2>
          ...
          <section>
            ...
            <ul className="space-y-3 mb-4">
              {sortedMembers.map((member) => (
                <li key={member.uid} className="flex items-center justify-between rounded-md border border-gray-200 dark:border-gray-700 px-3 py-2">
                  <div>
                    <p className="text-sm font-semibold text-gray-900 dark:text-gray-100">
                      {member.email}
                      {currentUserEmail && member.email === currentUserEmail && <span className="ml-2 text-xs text-green-600">(Du)</span>}
                    </p>
            ...
            {canManageMembers && (
              <form onSubmit={handleInvite} className="space-y-3 rounded-lg border border-dashed border-gray-300 dark:border-gray-600 p-4">
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                  ...
                  <button
                    ...
                    className="inline-flex justify-center rounded-md bg-green-600 px-4 py-2 text-sm font-semibold text-white hover:bg-green-700 disabled:opacity-60"
                  >
                    {inviteLoading ? 'Hinzufügen…' : 'Einladen'}
                  </button>
                </div>
                {inviteError && <p className="text-sm text-red-600 dark:text-red-400">{inviteError}</p>}
                {inviteMessage && <p className="text-sm text-green-600 dark:text-green-400">{inviteMessage}</p>}
              </form>
            )}
          </section>
        </main>
